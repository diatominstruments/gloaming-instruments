...
import { envStart, envRelease } from '../util.js';
 ...
 *
 *   await sampler.load([
 *     { sample: 'kits/909/kick.wav',  lo: 36, hi: 36, root: 36 },
 *     { sample: 'kits/909/snare.wav', lo: 38, hi: 38, root: 38 },
 *   ]);
...
export class Sampler extends Instrument {
  ...
  static params = {
    ...
    tune:    num(-24, 24, 0, { unit: 'st' }),
    attack:  num(0.001, 2, 0.001, { unit: 's', scale: 'log' }),
  ...
  async load(zones) {
    ...
    this.zones = await Promise.all(list.map(async ({ sample, root = 60, lo = 0, hi = 127 }) => {
      const buffer = sample instanceof AudioBuffer ? sample : await this.#fetch(sample);
      return { buffer, url: typeof sample === 'string' ? sample : null, root, lo, hi };
    }));
  }
  ...
  toJSON() {
    const samples = this.zones
      .filter((z) => z.url)
      .map(({ url, root, lo, hi }) => ({ sample: url, root, lo, hi }));
